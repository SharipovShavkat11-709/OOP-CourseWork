...
/**
 * FXML Controller class - Типы авто
 *
 * @author Admin
 */
public class CarTypeController implements Initializable {
@FXML
    private AnchorPane anchorPaneBaseDictionary;
    @FXML
    private TableView<CarType> tableViewDictionary;
    @FXML
    private TableColumn<CarType, Long> tableColumnID;
    @FXML
    private TableColumn<CarType, String> tableColumnName;
    @FXML
    ...
    QueryCarType query;
    private ObservableList<CarType> listTableView;
    ...
    private void buttonAddOnAction() {
        ...
        CarType addCarType = showAddEditDialog(true);
        //если значение не равно null, то
        if (addCarType != null) {
            //добавить данные в БД
            boolean add = query.add(addCarType);
    ...
    private void buttonEditOnAction() {
        ...
        CarType editCarType = showAddEditDialog(false);
        //если позиция отредактирована, то
        if (editCarType != null) {
            //обновить данные в БД
            boolean update = query.update(editCarType);
    ...
    private void buttonDeleteOnAction() {
        ...
        if (confirmation) {
            ...
            CarType deleteCarType = tableViewDictionary.getSelectionModel().getSelectedItem();
            //удалить данные из БД
            boolean delete = query.delete(deleteCarType.getId());
            ...
            if (!delete) {
                ...
                listTableView.remove(deleteCarType);
     ...
     * редактировать
     * @return в случае успеха - объект типа CarType, иначе - null
     */
    private CarType showAddEditDialog(boolean add) {
        try {
            //класс-загрузчик fxml файлов
            FXMLLoader loader = new FXMLLoader();
            //загружаем файл
            loader.setLocation(getClass().getResource("/fxml/manager/addEditCarType.fxml"));
            AnchorPane page = (AnchorPane) loader.load();
            Stage dialogStage = new Stage();

            if (add) {
                dialogStage.setTitle("Добавить тип авто");
            } else {
                dialogStage.setTitle("Изменить тип авто");
            }
            ...
            AddEditCarTypeController controller = (AddEditCarTypeController) loader.getController();

            if (!add) {
                controller.setCarType(tableViewDictionary.getSelectionModel().getSelectedItem());
            }

            //Отображаем диалоговое окно и ждем, пока пользователь его не закроет
            dialogStage.showAndWait();
            return controller.getCarType();
        } catch (IOException ex) {
            Logger.getLogger(CarTypeController.class.getName()).log(Level.SEVERE, null, ex);
            return null;
        }
    }   
